...
| `.claude/hooks/cmux-dispatch-hint.sh` | SessionStart **no-op** (exit 0). (구) cmux env 에서 dispatch-first 안내를 inject 했으나 "반사적 cmux dispatch" 압박 제거 정책(direct-edit 기본, cmux opt-in)에 따라 비활성화. 파일/등록은 propagate 호환 위해 유지. |
| `.claude/hooks/enforce-plan-mode.sh` | PreToolUse Write\|Edit. **/plan-dev plan mode 진입 강제** — plan-dev-session marker 활성 + plan mode 미진입 상태에서 Write/Edit 시도 시 exit 2 차단. 판정: PreToolUse stdin 의 `permission_mode` 필드. `permission_mode==plan` 이면 `<git-common-dir>/plan-dev-plan-mode-seen` flag 기록 후 allow (plan mode 진입 신호 = plan 파일 Write 가 이 경로로 들어옴). flag 존재(이 세션 plan mode 거침) → allow. `permission_mode==bypassPermissions` (dispatch 자식/명시 우회) → allow. 자식 worktree(git-dir≠git-common-dir) → skip. **마커 없음(비-plan-dev 세션) → no-op**. flag 는 `plan-dev-session.sh start`/`clear` 가 rm (매 세션 재진입 강제). 우회: `SKIP_PLAN_MODE_ENFORCE` / `DISABLE_PLAN_MODE_ENFORCE_HOOK`. 한계: plan reject 후에도 flag set 이면 통과 — 목적은 "plan mode 아예 미진입" catch. |
...
- ❌ cmux 환경이라고 / "진행" 명령받았다고 반사적으로 cmux dispatch — **direct-edit 기본, cmux opt-in**. dispatch 는 사용자 명시 요청(`--mode=cmux`/시각화) 또는 큰 병렬 작업 격리 가치 있을 때만. `track-cmux-edit-burst` 는 advisory only (디폴트 50, 차단 없음) — direct-edit 누적 자체는 정상. cmux dispatch 경로는 opt-in 으로 보존.
...
| `PLAN_MODE_SEEN_FILE` | auto | `enforce-plan-mode.sh` plan-mode-seen flag 경로 override (디폴트 마커 디렉토리/`plan-dev-plan-mode-seen`, 테스트 mock) |
