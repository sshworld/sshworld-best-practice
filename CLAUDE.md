...
| `scripts/cmux-title-chpwd.sh` | zsh `chpwd` hook — cd 마다 cmux surface title 을 `basename $PWD` 로 자동 rename (cmux 환경 `CMUX_SURFACE_ID` set 시만, 비-cmux no-op). `~/.zshrc` 에서 source. install.sh user scope 가 `~/.zshrc` 에 idempotent source block 추가 (marker `# >>> cmux-title-chpwd >>>`). `CMUX_BIN` env 로 mock. |
