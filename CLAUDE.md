...
| `hooks/notify-slice-done.sh` | Stop hook (cmux 자식 worktree 전용). 자식 turn 종료 시 마지막 assistant 텍스트에서 ✅/❌ 판정 → (a) `cmux notify` 로 부모 사이드바 알림 push, (b) `<git-common-dir>/cbp-slice-done-<branch sanitized: / → _>` done-marker 파일에 `$CMUX_SURFACE_ID` 기록 — `cmux-pane.sh reap` fast-path(`CBP_REAP_FAST_CHECK`) 의 소비 계약. 가드: `CMUX_WORKSPACE_ID` unset / 비-자식-worktree(git-dir==git-common-dir) / 비-`.worktrees/*` 인 경우 `CBP_NOTIFY_ANY_WORKTREE=1` 없으면 skip. jq 부재 시 판정 불가 `🔔 turn 종료` 로 강등. 모든 경로 exit 0(세션 안 막음). 우회: `SKIP_SLICE_DONE_NOTIFY=1`(1회) / `DISABLE_SLICE_DONE_NOTIFY=1`(영구). |
