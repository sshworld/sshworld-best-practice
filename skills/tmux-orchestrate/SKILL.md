...
```bash
# 1. shell 먼저
pane=$(${CLAUDE_PLUGIN_ROOT}/scripts/tmux-pane.sh launch zsh)

# 2. 자식 Claude 실행
${CLAUDE_PLUGIN_ROOT}/scripts/tmux-pane.sh send "claude" --pane=$pane
${CLAUDE_PLUGIN_ROOT}/scripts/tmux-pane.sh wait-idle --pane=$pane --idle=2 --timeout=30

# 3. 질문 전송 → 응답 대기
${CLAUDE_PLUGIN_ROOT}/scripts/tmux-pane.sh send "분석해줘: ..." --pane=$pane
${CLAUDE_PLUGIN_ROOT}/scripts/tmux-pane.sh wait-idle --pane=$pane --idle=5 --timeout=180

# 4. 회수
reply=$(${CLAUDE_PLUGIN_ROOT}/scripts/tmux-pane.sh capture --pane=$pane)
echo "$reply" | tail -50  # 마지막 응답 블록

# 5. 정리 (선택)
${CLAUDE_PLUGIN_ROOT}/scripts/tmux-pane.sh kill --pane=$pane
```
...
| 외부 tool 미설치, 본 repo 의 wrapper 만 존재 | `${CLAUDE_PLUGIN_ROOT}/scripts/tmux-pane.sh` 폴백 — 의존성 없음 |
| 둘 다 없음 | tmux 도 없는 환경 — `brew install tmux` + `uv tool install claude-code-tools` 안내 |
| cmux 안 surface (`CMUX_SOCKET_PASSWORD` set) | `${CLAUDE_PLUGIN_ROOT}/scripts/cmux-pane.sh` 사용 — launch/send/capture/wait-idle 동일 5단계 |

두 도구는 명령 표면이 정렬되어 있어 스크립트는 wrapper 변수로 추상화 가능:
```bash
W=$(command -v tmux-cli || echo "${CLAUDE_PLUGIN_ROOT}/scripts/tmux-pane.sh")
pane=$("$W" launch zsh)
```

cmux 도 동일 5단계 — wrapper 만 `${CLAUDE_PLUGIN_ROOT}/scripts/cmux-pane.sh` 로 바꿈:
```bash
W="${CLAUDE_PLUGIN_ROOT}/scripts/cmux-pane.sh"
pane=$("$W" launch zsh)
