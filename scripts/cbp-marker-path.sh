...
cbp_mask_volatile() {
  sed -E \
    ...
    -e 's/💰.*/<COST>/' \
    -e '/<SPIN>/ s/[0-9]+(\.[0-9]+)?s/<T>/g' \
    -e '/<COST>/ s/[0-9]+(\.[0-9]+)?s/<T>/g'
}
# ⚠️ 맨 초 단위(`42s`)는 **상태줄에서만** 마스킹한다.
# 전역으로 지우면 본문의 "테스트 42s 소요" 같은 실제 진행 변화까지 삼켜,
# 자식이 아직 일하는 중인데 idle 로 오판한다 — 조용히 틀리는 쪽이 더 위험하다.
# 경과시간(`10m 5s`)과 비용 라인은 본문에 나올 일이 없어 전역 처리해도 안전하다.
