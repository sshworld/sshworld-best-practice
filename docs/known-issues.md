...
| 18 | cmux dispatch 전반 (2026-07-08 관측) | cmux 자식 세션 4/4 회 crash 관측 — 비결정적 자식 사망 빈도가 높음. **subagent 폴백 경로는 미검증** (2026-08-14·08-18 두 세션이 시도했으나 `.gitignore` 의 `.claude/specs/*.spec.md` 로 spec 이 격리 worktree 에 안 따라가 중단, 부모 direct-edit 로 완주). 절차는 `commands/plan-dev/troubleshooting-dispatch.md` 의 'subagent 폴백' 에 명문화됨(본문 인라인) — **실측 1회 남음**. **2026-07-13~14 v1.5.0 이후 dispatch 4/4 첫 시도 성공 + 자식 3/3 완주 실측 (n 작음, 관찰 지속) — 상→중 하향** | 중 |
