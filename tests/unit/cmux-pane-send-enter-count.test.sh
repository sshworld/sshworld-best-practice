...
result=$(CMUX_BIN=echo bash "$SCRIPT" send "hello" --pane=workspace:1 --enter-count=3 2>/dev/null)
line_count=$(echo "$result" | wc -l | tr -d ' ')
check "--enter-count=3: echo 호출 4회" "4" "$line_count"

# ----------------------------------------------------------------
# 2. --enter-count=0 → echo 호출 1번 (send 만, send-key 없음)
result=$(CMUX_BIN=echo bash "$SCRIPT" send "hello" --pane=workspace:1 --enter-count=0 2>/dev/null)
line_count=$(echo "$result" | wc -l | tr -d ' ')
