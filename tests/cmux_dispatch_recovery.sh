...
# mock cmux 로 read-screen 응답 고정 → wait-idle idle 도달 + capture grep ✅ 패턴.
...
if [ "${CMUX_E2E:-0}" = "1" ] && [ -n "${CMUX_WORKSPACE_ID:-}" ]; then
  step 6 "[CMUX_E2E=1] 실 cmux — workspace 안 split + capture (사용자 화면에 surface 1개 추가됨)"
  unset CMUX_BIN
